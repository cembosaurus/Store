...
{
    public class AddItemsToCart_C : IRequest<IServiceResult<IEnumerable<CartItemReadDTO>>>
    {
        public int? UserId { get; set; }
        public IEnumerable<CartItemUpdateDTO> Items { get; set; }

        public class Validator : AbstractValidator<AddItemsToCart_C>
        {
            public Validator()
            {
                RuleFor(x => x)
                    .NotNull()
                    .WithMessage("- Add Items To Cart command model must NOT be NULL !");
                When(x => x != null, () => {
        ...
        public class AddItemsToCart_H : IRequestHandler<AddItemsToCart_C, IServiceResult<IEnumerable<CartItemReadDTO>>>
        {
            ...
            public async Task<IServiceResult<IEnumerable<CartItemReadDTO>>> Handle(AddItemsToCart_C request, CancellationToken cancellationToken)
            {
                var result = await _cartItemsService.AddItemsToCart(request.UserId ?? 0, request.Items);
