...
namespace Inventory.Services.Tools
{
    public class ServiceTools: IServiceTools
    {
        ...
        public ServiceTools(IMapper mapper, IServiceResultFactory resultFact)
        {
            _mapper = mapper;
        ...
        public IServiceResult<CatalogueItem> AddExtrasToCatalogItem(CatalogueItem catalogueItem, IEnumerable<int> Accessories, IEnumerable<int>? SimilarProducts)
        {
            ...
            if (catalogueItem == null)
                return _resultFact.Result<CatalogueItem>(null, false, "Null reference CatalogueItem !");

            if (Accessories == null && SimilarProducts == null)
                return _resultFact.Result<CatalogueItem>(null, false, "Argument is empty !");
            ...
            return _resultFact.Result(catalogueItem, true, intersected.Any() ? $"-->  There were found duplicated extras for catalogue item '{catalogueItem.ItemId}': '{catalogueItem.Item.Name}'. They were not written into DB: '{string.Join(",", intersected)}'" : "");
        }
        ...
        public IServiceResult<CatalogueItem> RemoveExtrasFromCatalogItem(CatalogueItem catalogueItem, IEnumerable<int> Accessories, IEnumerable<int>? SimilarProducts)
        {
            ...
            return _resultFact.Result(catalogueItem, true, missed.Any() ? $"-->  There were few attempts to remove non-existing extras from catalogue item '{catalogueItem.ItemId}': '{catalogueItem.Item.Name}'. They were ignored: '{string.Join(",", missed)}'" : "");
        }
