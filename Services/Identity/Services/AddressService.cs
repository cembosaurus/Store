...
namespace Identity.Services
{
    public class AddressService : IAddressService
    {
        ...
        public async Task<IServiceResult<AddressReadDTO>> AddAddressToUser(int userId, AddressCreateDTO addressDto)
        {
            ...
            var result = await _addressRepo.CreateAddress(address);

            if (result.State != EntityState.Added || _addressRepo.SaveChanges() < 1)
        ...
        public async Task<IServiceResult<AddressReadDTO>> DeleteAddress(int id)
        {
            ...
            var result = await _addressRepo.DeleteAddress(address);

            if (result.State != EntityState.Deleted || _addressRepo.SaveChanges() < 1)
